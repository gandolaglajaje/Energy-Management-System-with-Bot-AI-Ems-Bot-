...
function getCurrentWeekDays1() {
  ...
  const weekDays = [];
  for (let i = 0; i < 7; i++) {
      const day1 = new Date(startOfWeek);
      day1.setDate(startOfWeek.getDate() + i);
...
async function fetchDataForCurrentWeek1() {
  try {
      ...
      const firstDayOfWeek = new Date(currentDate.setDate(day1 - currentDayOfWeek + (currentDayOfWeek === 0 ? -6 : 1)));
      const lastDayOfWeek = new Date(currentDate.setDate(firstDayOfWeek.getDate() + 6));
      const startDate = `${year1}-${month1}-${firstDayOfWeek.getDate()}`;
      const endDate = `${year1}-${month1}-${lastDayOfWeek.getDate()}`;
